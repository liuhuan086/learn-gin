...
func GetAnArticle(id int) (article Article) {
	db.Where("id = ?", id).First(&article)
	db.Model(&article).Related(&article.Tag)
	return article
}

func AddAnArticle(data map[string]interface{}) bool {
	db.Create(&Article{
...
func EditAnArticle(id int, data interface{}) bool {
	db.Model(&Article{}).Where("id = ?", id).Updates(data)
	return true
}

func DeleteAnArticle(id int) bool {
